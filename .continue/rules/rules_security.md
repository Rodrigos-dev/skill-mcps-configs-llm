# SYSTEM RULES - GLOBAL CONTINUE DIRETRIZES ESTREITAS DE SEGURANÇA E ESCREVENTE
...
- **EXCEÇÃO**: É permitido o acesso ao diretório global de configuração do Continue localizado em `C:\Users\Acer\.continue\` para leitura de arquivos de configuração (`rules_security.md`, `config.yaml`, etc).
...
- Ao iniciar cada sessão e antes de enviar cada prompt para nuvem, o agente DEVE reler o arquivo de regras (rules_security.md) global(caso exista - ver caminho no config.yaml global) e no projeto(caso exista ver caminho no config.yaml local) para garantir que está seguindo as diretrizes mais recentes.
...
- Para ler arquivos globais do Continue, usar o caminho completo `C:\Users\Acer\.continue\` conforme a exceção da regra 1

## 8. Gestão de Memória do Projeto

- Ao iniciar cada sessão, o agente DEVE ler o arquivo `.continue/memory/memory.md` para obter contexto do projeto.
