...
1. Ler `C:\Users\Acer\.openclaude\CLAUDE.md`
2. Ler `C:\Users\Acer\.openclaude\memory\MEMORY.md`
3. **Criar nova sessão** (usando session-manager)
4. **Ler últimas 3 sessões** para contexto
5. Verificar se existe tarefa SDD ativa em `.openclaude/.tasks/`
6. Se existir, ler os arquivos de auditoria mais recentes:
...
7. Exibir a mensagem de confirmação

## Fluxo de Sessão (OBRIGATÓRIO)

### Passo 1: Criar Nova Sessão

1. Listar arquivos `session-*.json` em `C:\Users\Acer\.openclaude\sessions\`
...
3. Criar arquivo `session-XXX.json` com:
   ...
   ```
4. Salvar em `C:\Users\Acer\.openclaude\sessions\session-XXX.json`
