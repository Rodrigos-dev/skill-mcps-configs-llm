CLAUDE_CAMINHO = `C:\Users\Acer\.openclaude\` ou `C:\Users\Acer\.claude\`

# Regras de Segurança

> Regras obrigatórias para todas as sessões

## 1. Limitação de Escopo

- Acesso estritamente limitado à pasta raiz do projeto
- Proibido usar caminhos relativos de subida (`../`)
- Exceção: leitura de configurações globais em CLAUDE_CAMINHO
...
- **EXCEÇÃO**: É permitido o acesso ao diretório global de configuração do opencode localizado em CLAUDE_CAMINHO para leitura de arquivos de configuração (`rules_security.md`, `config.json`, etc).
...
- Para ler arquivos globais do claude, usar o caminho completo CLAUDE_CAMINHO conforme a exceção da regra 1

## 8. Gestão de Memória do Projeto

- Ao iniciar cada sessão, o agente DEVE ler o arquivo `CLAUDE_CAMINHO/memorys/memory.md` para obter contexto do projeto.
